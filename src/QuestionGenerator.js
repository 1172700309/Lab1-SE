...
class myLogger{
    constructor(){
        this.loggerHistory = new Array(0);
    }
     ...
     * @param time 时间
     * @param info 时间信息
     * @param content  时间具体内容
     */
    writeLogger(time, info, content){
        this.time = myLogger.getLocalTime(time);
        this.info = info;
        this.myContent = content;
        this.loggerHistory.push("time:"+this.time+",info:"+info+",content:"+content);
    }
    ...
    generateQuestion(){
        ...
        while (true){
            ...
            if(this.op===0&&this.firstNumber+this.secondNumber<100){
                logger.writeLogger(((new Date()).getTime()).toString(), "成功生成一道加法试题", this.firstNumber+"+"+this.secondNumber);
                ...
                logger.writeLogger(((new Date()).getTime()).toString(), "成功生成一道减法试题", fn+"-"+sn);
...
class Question{
    ...
    judgementAnswer(userAnswer){
        ...
        logger.writeLogger(((new Date()).getTime()).toString(), "用户对问题作答",
            "用户答案："+this.userAnswer+",回答情况："+this._answer === userAnswer);
 ...
 * @returns {boolean} 返回用户的答案是否正确
 */
function judgementAnswer(correctAnswer, userAnswer){
    logger.writeLogger(((new Date()).getTime()).toString(), "用户对问题作答",
        "用户答案："+userAnswer+",回答情况："+correctAnswer === userAnswer);
